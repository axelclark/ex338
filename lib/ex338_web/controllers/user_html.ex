defmodule Ex338Web.UserHTML do
  ...
  def show(assigns) do
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      <div class="flex flex-col text-center bg-white rounded-lg shadow-sm col-span-1">
        <div class="flex flex-col flex-1 p-8">
          <.user_profile_image user={@user} class="shrink-0 w-32 h-32 mx-auto bg-black rounded-full" />
          <h3 class="mt-6 font-medium text-gray-900 tex leading-5">{@user.name}</h3>
          <dl class="flex flex-col justify-between grow mt-1">
            ...
            <div class="flex justify-center mt-4">
              <span class="inline-flex rounded-md shadow-xs">
                <.link
                  ...
                  class="inline-flex items-center px-4 py-2 text-base font-medium text-white bg-indigo-600 border border-transparent leading-6 rounded-md hover:bg-indigo-500 focus:outline-hidden focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150"
                >
                  Update Info
                </.link>
              </span>
