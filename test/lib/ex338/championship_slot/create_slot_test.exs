defmodule Ex338.ChampionshipSlot.CreateSlotTest do
  use Ex338.ModelCase
  alias Ex338.{ChampionshipSlot.CreateSlot, ChampionshipSlot, FantasyTeam, Repo}
  ...
  describe "create_slots_from_positions/1" do
    test "creates roster slots from positions" do
      ...
      ua = insert(:roster_position, fantasy_player: player_c,
       fantasy_team: other_team, status: "active", position: "Unassigned")
      teams =
        FantasyTeam
      ...
      {:ok, _results} =
        CreateSlot.create_slots_from_positions(teams, championship.id)
      slots = Repo.all(ChampionshipSlot)

      assert Enum.map(slots, &(&1.roster_position_id)) ==
        [primary.id, flex.id, ua.id]
    end
  end
